...
import {
    Search, X, Code2, Flame, Target, Zap,
...
const Pill = ({ label, active, onClick, count }) => (
    <button
        onClick={onClick}
        className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider whitespace-nowrap transition-all duration-200"
        style={active ? {
    ...
    >
        ...
        {count !== undefined && (
            <span className="px-1.5 py-0.5 rounded-md text-[9px] font-black"
                style={{ background: active ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.06)", color: active ? "#fff" : "#94a3b8" }}>
                {count}
            </span>
        )}
    </button>
...
const HeroStat = ({ icon, label, value, color }) => (
    <div className="flex items-center gap-3 p-4 rounded-2xl transition-all duration-200 hover:scale-[1.02]"
        style={{
...
const SortSelect = ({ value, onChange }) => (
    <div className="relative">
        <select
            ...
            className="appearance-none pl-4 pr-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider outline-none cursor-pointer transition-all"
            style={{
                background: "var(--pill-bg, rgba(0,0,0,0.04))",
                border: "1px solid var(--pill-border, rgba(0,0,0,0.08))",
...
export default function CompanyGrid({ onSelectCompany }) {
    ...
    const filtered = useMemo(() => {
        ...
        switch (sort) {
            ...
            case "salary": list.sort((a, b) => {
                const n = s => parseInt((s || "0").split("–").pop().replace(/\D/g, "")) || 0;
    ...
    const activeFilterCount = [category !== "All", tier !== "All", difficulty !== "All", activeOnly, !!search].filter(Boolean).length;
            ...
            <div className="mb-10">
                ...
                <div className="flex items-end justify-between gap-6">
                    ...
                    <motion.p
                        ...
                        <a href="https://github.com/snehasishroy/leetcode-companywise-interview-questions"
                ...
                <div className="flex gap-3 mb-0">
                    ...
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-9 py-2.5 rounded-xl text-sm font-semibold outline-none transition-all text-slate-800 dark:text-white placeholder-slate-400"
                            style={{
                    ...
                    <button
                        ...
                        Filters
                        {activeFilterCount > 0 && (
                            <span className="w-4.5 h-4.5 px-1.5 py-0.5 rounded-full bg-white/25 text-white text-[9px] font-black">
                                {activeFilterCount}
                            </span>
                        )}
                    </button>
                    ...
                    {showFilters && (
                        <motion.div
                            ...
                            <div className="pt-4 mt-4 space-y-4"
                                style={{ borderTop: "1px solid var(--divider, rgba(0,0,0,0.06))" }}>

                                {[
                                    {
                                        label: "Category", options: CATEGORIES, value: category, set: setCategory,
                                        count: c => c === "All" ? COMPANIES.length : COMPANIES.filter(co => co.category === c).length
                                    },
                                    { label: "Tier", options: TIERS, value: tier, set: setTier },
                                    { label: "Difficulty", options: DIFFICULTIES, value: difficulty, set: setDifficulty },
                                ].map(group => (
                                    <div key={group.label}>
                                        <p className="text-[9px] font-black text-slate-400 dark:text-white/25 uppercase tracking-widest mb-2">
                                            {group.label}
                                        </p>
                                ...
                                <div className="flex items-center gap-4 pt-1">
                                    ...
                                    {hasFilters && (
                                        <button onClick={clearAll}
                                            className="text-[10px] font-black text-rose-500 hover:text-rose-600 flex items-center gap-1 transition-colors ml-auto">
                                            <X size={11} /> Clear All
                                        </button>
                                    )}
                                </div>
            ...
            <AnimatePresence mode="popLayout">
                {filtered.length > 0 ? (
                    ...
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="flex flex-col items-center justify-center py-32 rounded-[32px]"
                        style={{
                            background: "var(--empty-bg, rgba(0,0,0,0.02))",
                            border: "2px dashed var(--divider, rgba(0,0,0,0.07))",
                        }}>
                        <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4"
                            style={{ background: "var(--pill-bg, rgba(0,0,0,0.05))" }}>
                        ...
                        <button onClick={clearAll}
                            className="px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider text-white transition-all hover:scale-105"
